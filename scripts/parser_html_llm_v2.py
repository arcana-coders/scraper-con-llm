...
# === CONF    print(f"📖 Leyendo el contenido crudo de {html_path.name}...")
...
PROMPT_LIMPIEZA = """
Eres un experto desarrollador frontend. Tu tarea es analizar el siguiente código HTML de una página de Amazon Seller Central y aislar la sección principal que contiene la lista de pedidos.
Busca una tabla o un contenedor principal que liste múltiples productos con sus detalles como ID de pedido, fecha, nombre del producto, ASIN, SKU, etc.
Una vez que la encuentres, devuelve ÚNICAMENTE el código HTML de esa sección (la tabla o el div contenedor principal). No incluyas el <head>, <body>, menús de navegación, encabezados o pies de página. Solo el HTML de la sección de pedidos.
No expliques nada. Solo devuelve el fragmento de código HTML.
"""
...
def guardar_datos(datos: list[dict]):
    ...
    todas_las_keys = set()
    for item in datos:
        todas_las_keys.update(item.keys())
    ...
    with open(OUTPUT_CSV_CONSOLIDADO, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=sorted(list(todas_las_keys)))
        writer.writeheader()
...
def main():
    ...
    print(f"\n📖 Leyendo el contenido crudo de {html_path.name}...")
    html_crudo = html_path.read_text(encoding="utf-8")
